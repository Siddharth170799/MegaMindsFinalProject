...
const authenticateUser = async (email, password) => {   //// authenticate function which checks whetehr the user exists here
...
const isPasswordValid = await bcrypt.compare(password, details.Password);/// here the user password is being compared with already save dpassword
                                 ///// this bcrypt compare applies rhe same salt to the passowrd and checks whether it is same                            

  if (!isPasswordValid) {
    console.log("Invalid password");
    return null;     //// if password is not valid then return null
  }

  console.log("User authenticated successfully", details);
  return details;              //// returning the user object if exists
};
...
SignUpRoute.post(
  "/postSignUpDetails",
   [
    body("Name")            /// express-validator to check the reqbody
      .isString()
  ...
  async (req, res) => {
    try {
      ...
      if (userObject) {
        ...
        const token = jwt.sign({ userId: userId }, "my_secret_key", {   /// here if userobject exists the jwt token 
                                 ////is being created using jwt.verify
          expiresIn: '1hr'
        });
        res.send({ token });
      } else {
